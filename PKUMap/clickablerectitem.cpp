...
ClickableRectItem::ClickableRectItem(const QString &name, const QString &detail, QGraphicsItem *parent)
    : QGraphicsEllipseItem(parent), m_name(name), m_detail(detail)
{
    setRect(0, 0, 10, 10);
    setBrush(QColor(255, 0, 0, 140));
...
void ClickableRectItem::mousePressEvent(QGraphicsSceneMouseEvent *event) {
    ...
    emit clicked(m_name, m_detail);  // 发射信号
}
