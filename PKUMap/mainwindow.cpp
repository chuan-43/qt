...
void sqlite_Init()
{
    ...
    query.exec("DROP TABLE IF EXISTS favorites");
    query.exec("DROP TABLE IF EXISTS comments");*/

    //create excle
    QString createsql=QString("create table if not exists user("
                                "username ntext unique primary key not NULL,"
                                "password ntext not NULL)");

    // 新增收藏表
    QString createFavorites =QString("CREATE TABLE IF NOT EXISTS favorites("
                              "id INTEGER PRIMARY KEY AUTOINCREMENT,"
                              "username TEXT NOT NULL,"          // 关联用户ID
                              "spot_name TEXT NOT NULL,"          // 景点名称
                              "collected INTEGER DEFAULT 0,"
                              "FOREIGN KEY(username) REFERENCES user(username))");

    //评论表
    QString createComments = "CREATE TABLE IF NOT EXISTS comments("
                             "id INTEGER PRIMARY KEY AUTOINCREMENT, "
    ...
    if(!query.exec(createsql)){
        qDebug()<<"sql table create error";
    }else if(!query.exec(createFavorites)){
        qDebug()<<"fav table create error";
    }else if(!query.exec(createComments)){
        qDebug()<<"com table create error";
    }
    else{
...
void MainWindow::on_btn_signin_clicked()
{
    ...
    else
    {
        ...
        msgBox.exec();
        GlobalData::instance().setCurrentUserId(username);
        //登录成功后可以跳转到其他页面
